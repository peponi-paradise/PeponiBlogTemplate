...
- `using{:cs}`문은 [IDisposable](https://learn.microsoft.com/ko-kr/dotnet/api/system.idisposable?view=net-7.0), [IAsyncDisposable](https://learn.microsoft.com/ko-kr/dotnet/api/system.iasyncdisposable?view=net-7.0) 객체가 scope 끝에 바로 dispose될 수 있도록 한다.
...
- `using`문은 다음과 같이 사용한다.
    ```cs showLineNumbers title="C# | Hello.cs" {3-4}
...
- 아래와 같이, 하나의 `using`문에서 여러 인스턴스를 생성할 수도 있다.
    ```cs showLineNumbers{1234} {1-2} title="C# | Hello.cs"
